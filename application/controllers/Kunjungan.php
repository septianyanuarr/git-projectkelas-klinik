...
public function print()
    {
        $data['kunjungan'] = $this->M_kunjungan->tampil_data('kunjungan')->result_array();
        $this->load->view('kunjungan/print', $data);
    }
...
{
    {
        ...
        $data['kunjungan'] = $this->M_kunjungan->tampil_data('kunjuingan')->result_array();

        $this->load->view('kunjungan/laporan_pdf', $data);
        ...
        $this->dompdf->load_html($html);
        $this->dompdf->render();
        $this->dompdf->stream("kunjungan.pdf", array('Attachment' => 0)); // nama file pdf yang dihasilkan
        }
    }
    public function excel()
    {
    $data = array( 'title' => 'Laporan Data Kunjungan',
    'laporan' => $this->db->query("select berobat.id_pasien, berobat.tgl_berobat, pasien.nama_pasien, pasien.jenis_kelamin, pasien.umur, berobat.keluhan_pasien, berobat.hasil_diagnosa, berobat.penatalaksanaan FROM berobat JOIN pasien ON berobat.id_pasien = pasien.id_pasien")->result_array());
    $this->load->view('kunjungan/export_excel', $data);
    }
}